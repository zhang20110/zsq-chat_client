 ...
 * 发送消息方是否知道对方已读暂不实现
 */
async function sendMessageStatus(data) {
    const mesType = 'sendMessageStatus';
    const { ids } = data;
    // // 更新消息为已读
    console.log('sendMessageStatus ids: ', ids);
    const updateData = await Message_1.default.find({
        _id: { $in: ids },
        status: 'unread'
    ...
    // 通知发送消息人
    if ((0, websocket_1.isLine)(from)) {
        (0, websocket_1.sendMes)(from, {
            mesType,
            updateIds
        });
    }
...
exports.default = sendMessageStatus;
